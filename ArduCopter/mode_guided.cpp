...
void ModeGuided::run()
{
    ...
    case SubMode::TrajStream:
        trajstream_control_run();
        break;
...
// initialise guided mode's onboard-MPC trajectory-stream controller
// mirrors pva_control_start() but sizes the NE/D limits to the plan envelope,
// holds heading (crab) and feeds the buffered trajectory RAW (no S-curve).
void ModeGuided::trajstream_control_start()
{
    ...
    const float ne_speed_ms = MAX(wp_nav->get_default_speed_NE_ms(), 12.0f);
    const float ne_accel_mss = MAX(wp_nav->get_wp_acceleration_mss(), GRAVITY_MSS * tanf(radians(35.0f)));
    pos_control->NE_set_max_speed_accel_m(ne_speed_ms, ne_accel_mss);
    pos_control->NE_set_correction_speed_accel_m(ne_speed_ms, ne_accel_mss);
    ...
    _trajstream_entry_pos_d_m = pos_control->get_pos_estimate_NED_m().z;
    ...
    {
        const Vector2f vel_ne_ms = pos_control->get_vel_estimate_NED_ms().xy();
        pos_control->set_drag_accel_ff_NE_mss(vel_ne_ms * (_traj.drag_k * vel_ne_ms.length()), true);
    }

    // reset the Tier-2 station-keep latch
    _trajstream_hold_active = false;

    // crab: hold current heading, do not turn the nose along the track
    auto_yaw.set_mode(AutoYaw::Mode::HOLD);

    // guided position target is not a terrain altitude
    guided_is_terrain_alt = false;

    // enter the trajectory-stream submode
    guided_mode = SubMode::TrajStream;
}

// trajstream_control_run - replays the buffered onboard-MPC trajectory
// called from guided_run at the main loop rate (~400 Hz)
void ModeGuided::trajstream_control_run()
{
    // if not armed set throttle to zero and exit immediately
    if (is_disarmed_or_landed()) {
        // do not spool down tradheli when on the ground with motor interlock enabled
    ...
    pos_ned_m.z = _trajstream_entry_pos_d_m;

    const uint32_t now_ms = millis();
    const uint32_t age_ms = now_ms - _traj.commit_ms;

    float posN, posE, velN, velE, accN, accE;
    if (_traj.active_n == 0 || age_ms > _traj.duration_ms() + get_timeout_ms()) {
        ...
        if (!_trajstream_hold_active) {
            _trajstream_hold_active = true;
            _trajstream_hold_pos_ne_m = pos_control->get_pos_estimate_NED_m().xy();
        }
        pos_ned_m.xy() = _trajstream_hold_pos_ne_m;
        vel_ned_ms.zero();
        accel_ned_mss.zero();
    } else {
    ...
    {
        const Vector2f vel_ne_ms = pos_control->get_vel_estimate_NED_ms().xy();
        pos_control->set_drag_accel_ff_NE_mss(vel_ne_ms * (_traj.drag_k * vel_ne_ms.length()), false);
    }

    // feed the controller RAW - no S-curve reshaping (this is the clean seam,
    // identical to how AC_WPNav feeds AC_PosControl)
    pos_control->set_pos_vel_accel_NED_m(pos_ned_m, vel_ned_ms, accel_ned_mss);
    ...
    if (now_ms - _trajstream_last_log_ms >= 100) {
        _trajstream_last_log_ms = now_ms;
        copter.Log_Write_Guided_Position_Target(guided_mode, guided_pos_target_ned_m, guided_is_terrain_alt, guided_vel_target_ned_ms, guided_accel_target_ned_mss);
    }
#endif
...
bool ModeGuided::mpc_trajectory_chunk(uint16_t traj_id, uint16_t num_points, uint16_t start_index, uint8_t count, uint16_t dt_ms,
                                      ...
                                      uint32_t time_boot_ms, float drag_k)
{
    ...
    if (guided_mode == SubMode::TakeOff && !takeoff_complete) {
        return false;
    }
    ...
    if (_traj.pending_next == _traj.pending_n) {
        ...
        const float node_alt_d_m = (guided_mode == SubMode::TrajStream) ?
            (float)_trajstream_entry_pos_d_m : pos_control->get_pos_estimate_NED_m().z;
        ...
        copter.Log_Write_Guided_Position_Target(SubMode::TrajStream,
            Vector3p{(postype_t)_traj.active[0].pos_n, (postype_t)_traj.active[0].pos_e, _trajstream_entry_pos_d_m}, false,
            Vector3f{_traj.active[0].vel_n, _traj.active[0].vel_e, 0.0f},
            Vector3f{_traj.active[0].acc_n, _traj.active[0].acc_e, 0.0f});
#endif
        // auto-start the submode on first commit (mirrors how the PVA path
        // switches submode); subsequent commits just re-base the replay cursor
        if (guided_mode != SubMode::TrajStream) {
            trajstream_control_start();
        }
        return true;
    }
...
void ModeGuided::TrajBuffer::begin(uint16_t id, uint16_t n, uint16_t dt, float drag_k_perm)
{
    pending_id = id;
...
bool ModeGuided::TrajBuffer::put(uint16_t index, float pos_n, float pos_e, float vel_n, float vel_e, float acc_n, float acc_e)
{
    if (index >= TRAJ_MAX) {
        return false;
    }
...
void ModeGuided::TrajBuffer::commit(uint32_t base_ms)
{
...
uint32_t ModeGuided::TrajBuffer::duration_ms() const
{
    if (active_n == 0) {
        return 0;
    }
...
bool ModeGuided::TrajBuffer::sample(float elapsed_s, float &posN, float &posE, float &velN, float &velE, float &accN, float &accE) const
{
    if (active_n == 0) {
        return false;
    }
