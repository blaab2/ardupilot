...
// The controller itself lives in the copter-level MPCTrajReplay helper
// (mpc_replay.cpp — the Step-E code moved verbatim so AUTO can share it);
// this wrapper only keeps the guided submode bookkeeping.
...
void ModeGuided::trajstream_control_run()
{
    ...
    // the replay controller (sampling, drag-FF lean map, Tier-1/Tier-2
    // fallback, GUIP logging, attitude output) is the shared helper
    copter.mpc_replay.run();

    // mirror the replayed targets into the guided target statics for
    // telemetry / wp reporting (get_wp, POSITION_TARGET_LOCAL_NED)
    guided_pos_target_ned_m = copter.mpc_replay.target_pos_NED_m();
    guided_vel_target_ned_ms = copter.mpc_replay.target_vel_NED_ms();
    guided_accel_target_ned_mss = copter.mpc_replay.target_accel_NED_mss();
}
